...
public class WalletAppService
{
    ...
    public void AddTransaction(Guid walletId, TransactionDto dto)
    {
        ...
        if (dto.Type == "Expense" && wallet.CurrentBalance.Amount < dto.Amount)
    ...
    public List<Transaction> GetTopExpensesByMonthAndYearDescending
        (Guid walletId, int month, int year, int topCount = 3)
    {
        var transactions = GetTransactionsByMonthAndYearAscending(walletId, month, year);
...
public class WalletAppService
{
    ...
    public void AddTransaction(Guid walletId, TransactionDto dto)
    {
        ...
        wallet.AddTransaction(transaction); // бизнес-правила внутри агрегата
        _repository.AddTransaction(transaction); // сохраняем транзакцию
    }
